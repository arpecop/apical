...
function name(page) {
    ...
    request.get('https://pouchdb.herokuapp.com/bgusers/' + page, function (exm, xx, body) {
        ...
        let counter = 0;
        eachAsync(JSON.parse(body).docs, function (item, index, done) {

            //console.log(_.shuffle(pages)[0].access_token);
            request.get('https://graph.facebook.com/' + item.id + '?fields=first_name&access_token=' + _.shuffle(pages)[0].access_token, function (exm, xx, body) {
