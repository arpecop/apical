...
const pinterest = pintetez.init(
  'AT3u7ZwNxWQpVASg6-MmSf6l8y56FLrVnW7SARtD-s__umBBdgAAAAA',
);
    ...
    .api('pins', {
      ...
      body: {
        ...
        image_url: json.fullimg
          ? json.fullimg
          : 'http://www.vtc.edu/sites/default/files/news-3.jpg',
      },
    })
    .then((jsonx) => {
      request.get(
        `https://developers.pinterest.com/widget/pins/${jsonx.data.id}/`,
        (err, ser, body) => {
          if (!err && JSON.parse(body).data) {
            callback({
              _id: `${new Date().getTime()}_${Math.floor(Math.random() * 10 + 1)}`,
              url: JSON.parse(body).data.image.original.url.replace(
                'originals',
                '236x',
              ),
...
function crunch(id, callback) {
  Feed.load('http://feeds.feedburner.com/TechCrunch/', (err, rss) => {
    async.eachSeries(
      ...
      (item, cb) => {
        const json = item;
        json.fullimg = item.media.thumbnail
          ? item.media.thumbnail[0].url[0].split('?')[0]
          : 'https://tctechcrunch2011.files.wordpress.com/2017/03/tc-equity-podcast-ios.jpg';
        json.provider = 'TechCrunch';
        json.source = json.url;
        json.media = null;
        json.description = striptags(item.description).replace(
          'Read More',
          '',
        );
...
function upworthy(id, callback) {
  Feed.load('http://feeds.feedburner.com/upworthy', (err, rss) => {
    async.eachSeries(
      ...
      (item, cb) => {
        const json = item;
        json.fullimg = item.enclosures
          ? item.enclosures[0].url.split('?')[0]
          : 'https://www.upworthy.com/assets/social-eyecatcher-orange-0a6d6dca485d6e1c339cae4cfc777544.png';
...
function distractify(x, callback) {
  request.get(
    'http://distractify.com/api/0.1/channels/slug/trending/resources/latest/1/10',
    (er, ass, body) => {
      if (!er && body.length > 200) {
        async.eachSeries(
          JSON.parse(body).pkg.resources[0][1],
          (item, cb) => {
            // console.log(item);
            const json = {};
...
function buzz(x, callback) {
  request.get(
    'https://www.buzzfeed.com/site-component/v1/en-us/morebuzz?page_size=5&page=1',
    (er, ass, body) => {
        ...
        async.eachSeries(
          ...
          (item, cb) => {
            ...
            db.db1.get(json.uid, (err, doc) => {
              if (err) {
                request.get(
                  `https://graph.facebook.com/?id=${json.source}&access_token=${process.env.article_token}`,
                  (er, ass, body) => {
...
function newsapix(source, callback) {
  request.get(
    `https://newsapi.org/v1/articles?source=${source.src}&sortBy=${source.get}&apiKey=d734ebaa11aa4ad0b2df9e074d202869`,
    (er, ass, body) => {
      if (!er && body.length > 100) {
        async.eachSeries(
          JSON.parse(body).articles,
          (item, cb) => {
            const json = {};
            json.title = item.title;
  ...
  async.eachSeries(
    ...
    (err, results) => {
      db.get(
        ...
        (e, doc) => {
          console.info(
            `posting scheduled promo last post ${doc.docs[0].id} ${doc.docs[0].title}`,
          );
          promo.post(
            `news/${doc.docs[0].id}`,
