    ...
    if (arritem[0]) {
      client
        .post('statuses/update', {
          status: `https://www.izteglisi.club/app/news/${arritem[0]._id} ${arritem[0].title}`,
        }).then((tweet) => {
          console.log('posted');
          resolve();
        }).catch((error) => {
                ...
                .then((doc) => {
                  request.post('https://graph.facebook.com/', {
                    form: {
                      ...
                      id: `https://www.izteglisi.club/fb/izvestie/g/pix/${doc.rows[0].id}`,
                      scrape: true,
                    },
                  }, (error, d, body) => {
                    request.post(
                      ...
                      {
                        form: {
                          link: `https://www.izteglisi.club/fb/izvestie/g/pix/${doc.rows[0].id}`,
                          access_token: page.access_token,
                        },
                      },
                      (e, m, body) => {
                        ...
                        if (JSON.parse(body).error) {
                          request.post(
                            ...
                            {
                              form: {
                                caption: `${doc.rows[0].doc.title} : https://www.izteglisi.club/fb/izvestie/g/pix/${doc.rows[0].id}`,
                                access_token: page.access_token,
